(function($) {
  ...
  var Combogrid = function ( elem, options ) {
		var o = $.extend({}, $.fn.combogrid.defaults, options),
				that = $(elem),
				$source      = that,
				$container   = $(template()),
		...
		function template(){
			if (o.style=='bs3')
				return '<div class="combogrid-container"><div class="input-group"> <input type="text" autocomplete="off" class="form-control" /> <input type="hidden" /><div class="input-group-btn"><span class="btn btn-default dropdown-toggle" data-dropdown="dropdown"> <span class="caret" /> </span> </div> </div> </div>';
		}
  ...
  $.fn.combogrid = function(option) {
    ...
    return this.each(function() {
			...
			var that = $(this),
					data = that.data('combogrid'),
					options = typeof option == 'object' && option;
					
			// console.log('data-combogrid = '+data);
      if(!data) {that.data('combogrid', (data = new Combogrid(this, options)));}
			// console.log(data);
