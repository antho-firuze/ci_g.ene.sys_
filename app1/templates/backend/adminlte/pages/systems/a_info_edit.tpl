...
<script>
	...
	{* col.push(BSHelper.Input({ horz:false, type:"date", label:"Valid From", idname:"valid_from", cls:"auto_ymd", format:"datetime", required: false })); *}
	{* col.push(BSHelper.Input({ horz:false, type:"text", label:"Valid From", idname:"valid_from", })); *}
	...
	$.getJSON($("#select_valid_orgtrx").attr('url'), {}, function(result){ 
		if (!isempty_obj(result.data.rows)) { 
			$.each(result.data.rows, function(i, item) {
				$("#select_valid_orgtrx").append('<option value="' + item.id + '">' + item.code_name + '</option>');
			});
