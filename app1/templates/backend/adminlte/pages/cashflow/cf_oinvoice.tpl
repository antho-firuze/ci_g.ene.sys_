...
<script>
	...
	function actualization(data) {
		...
		col.push("<h3>"+(data.is_receipt == '1' ? "AR" : "AP")+" Doc No : <br>"+data.doc_no_ar_ap+"</h3>");
			...
			buttons:[{ 
				cssClass: 'btn-primary', label: 'Submit', hotkey: 13, action: function(dialog) {
					...
					if (form1.validator('validate').has('.has-error').length === 0) {
						...
						var io = data.is_receipt == '1' ? '_i' : '_o';
						$.ajax({ url: $url_module+io+'_actualization', method: "OPTIONS", async: true, dataType: 'json',	data: form1.serializeJSON(),
			...
			buttons:[{ 
				cssClass: 'btn-primary', label: 'Submit', hotkey: 13, action: function(dialog) {
					...
					if (form1.validator('validate').has('.has-error').length === 0) {
						...
						var io = data.is_receipt == '1' ? '_i' : '_o';
						$.ajax({ url: $url_module+io+'_adjustment', method: "OPTIONS", async: true, dataType: 'json',	data: form1.serializeJSON(),
							success: function(data) {
								BootstrapDialog.show({ closable: false, message:data.message, 
									buttons: [{ label: 'OK', hotkey: 13, action: function(dialogRef){ dialogRef.close(); } }],
								});
