...
<script>
	...
	col.push(BSHelper.Combobox({ horz:false, label:"Requsition No", label_link:"{$.const.PAGE_LNK}?pageid=91", textField:"doc_no", idname:"requisition_id", url:"{$.php.base_url('cashflow/cf_requisition')}?for_purchase_order=1,having=qty", remote: true, required: true }));
	...
	col.push(BSHelper.Combobox({ horz:false, label:"Price List", label_link:"{$.const.PAGE_LNK}?pageid=50", idname:"pricelist_id", url:"{$.php.base_url('sales/m_pricelist')}?filter=is_sotrx='0'", remote: true, required: true }));
	row.push(subCol(6, col)); col = [];
