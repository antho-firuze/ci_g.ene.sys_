      ...
      <ul class="sidebar-menu">
		...
		{foreach $menus as $menu}
			{if ($menu_id1 != $menu->menu_id1 && !empty($menu->menu_id1))}
				...
				<li class="treeview">
					<a href="{$menu->url1}" data-menu_id="{$menu->menu_id1}"><i class="fa {$menu->icon1 !: 'fa-circle'}"></i>
				...
				{if ($menu_id2 != $menu->menu_id2 && !empty($menu->menu_id2)) }
					<ul class="treeview-menu">
						<li class="{if ($menu->name2 == 'User')} active {/if}">
							<a href="{$menu->url2}" data-menu_id="{$menu->menu_id2}"><i class="fa {$menu->icon2 !: 'fa-circle-o'}"></i> {$menu->name2} 
							{if !empty($menu->menu_id3)} <i class="fa fa-angle-left pull-right"></i>{/if}
							</a>
					
					{if ($menu_id3 != $menu->menu_id3 && !empty($menu->menu_id3)) }
						<ul class="treeview-menu">
							<li><a href="{$menu->url3}" data-menu_id="{$menu->menu_id3}"><i class="fa {$menu->icon2 !: 'fa-circle-o'}"></i> {$menu->name3}</a>
			...
			{elseif ($menu_id1 == $menu->menu_id1)}
				...
				{if ($menu_id2 != $menu->menu_id2 && !empty($menu->menu_id2)) }
					<li><a href="{$menu->url2}" data-menu_id="{$menu->menu_id2}"><i class="fa fa-circle-o"></i> {$menu->name2}</a>
					
					{var $close_menu2 = 1}
					{var $menu_id2 = $menu->menu_id2}
					
				{elseif ($menu_id2 == $menu->menu_id2)}
					<li><a href="{$menu->url3}" data-menu_id="{$menu->menu_id3}"><i class="fa fa-circle-o"></i> {$menu->name3}</a>
