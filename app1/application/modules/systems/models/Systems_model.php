	...
	function getMenuByRoleId_old2($role_id)
	{
		if ($role_id) {
			...
			from (
				...
				exists (
					select * from (
						select am.id, am.name, am.is_parent, am.line_no, am.icon, am.type, am.parent_id, 
						case when arf.role_id is not null then arf.role_id when arp.role_id is not null then arp.role_id  when arw.role_id is not null then arw.role_id else null end as role_id 
						from a_menu am
						left join a_role_form arf on am.form_id = arf.form_id and arf.is_active = '1' and arf.is_deleted = '0' and arf.role_id = $role_id
						left join a_role_process arp on am.process_id = arp.process_id and arp.is_active = '1' and arp.is_deleted = '0' and arp.role_id = $role_id
						left join a_role_window arw on am.window_id = arw.window_id and arw.is_active = '1' and arw.is_deleted = '0' and arw.role_id = $role_id
						where am.is_submodule = '0' and am.is_active = '1' and am.is_deleted = '0'
					) basemenu where role_id is not null and parent_id = am.id
				)
			) am1
			left join (
				...
				left join a_role_form arf on am.form_id = arf.form_id and arf.is_active = '1' and arf.is_deleted = '0' and arf.role_id = $role_id
				left join a_role_process arp on am.process_id = arp.process_id and arp.is_active = '1' and arp.is_deleted = '0' and arp.role_id = $role_id
				left join a_role_window arw on am.window_id = arw.window_id and arw.is_active = '1' and arw.is_deleted = '0' and arw.role_id = $role_id where am.is_submodule = '0' and am.is_active = '1' and am.is_deleted = '0'
			) am2 on am1.id = am2.parent_id and am2.role_id is not null
			left join (
				select am.id, am.name, am.is_parent, am.line_no, am.icon, am.type, am.parent_id, 
				case when arf.role_id is not null then arf.role_id when arp.role_id is not null then arp.role_id  when arw.role_id is not null then arw.role_id else null end as role_id 
